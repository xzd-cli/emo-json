...
function managerConfig(argv) {
  const action = argv._[1] || 'ls'
  if(action === 'ls' || action === 'list') {
    require(path.resolve(__dirname, `list_config`))()
  } else if (action === 'set') {
    let key = argv._[2], value = argv._[3];
    require(path.resolve(__dirname, 'set_config'))(key, value)
  }
}
...
module.exports = function (argv) {
  ...
  if(source === 'c' || source === 'config') {
    managerConfig(argv)
    return
  }

  if(argv.ws) {
    websocketServer.create(argv);
    return
  }
