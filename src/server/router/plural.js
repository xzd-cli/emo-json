...
module.exports = (db, name, opts, configObject) => {
  ...
  function list(req, res, next) {
    ...
    // parameters
    let q = req.query[configObject?.q || 'q']
    let _start = req.query[configObject?._start || '_start']
    let _end = req.query[configObject?._end || '_end']
    let _page = req.query[configObject?._page || '_page']
    const _sort = req.query[configObject?._sort || '_sort']
    const _order = req.query[configObject?._order || '_order']
    let _limit = req.query[configObject?._limit || '_limit']
    const _embed = req.query[configObject?._embed || '_embed']
    const _expand = req.query[configObject?._expand || '_expand']
    delete req.query[configObject?.q || 'q']
    delete req.query[configObject?._start || '_start']
    delete req.query[configObject?._end || '_end']
    delete req.query[configObject?._sort || '_sort']
    delete req.query[configObject?._order || '_order']
    delete req.query[configObject?._limit || '_limit']
    delete req.query[configObject?._embed || '_embed']
    delete req.query[configObject?._expand || '_expand']
    ...
    res.locals.data = adapterConfig(chain.value(), res.getHeader('X-Total-Count'))
    next()
  }
  ...
  function show(req, res, next) {
    ...
    if (resource) {
      ...
      res.locals.data = adapterConfig(clone)
    }
  ...
  function create(req, res, next) {
    ...
    res.status(201)
    res.locals.data = adapterConfig(resource)
